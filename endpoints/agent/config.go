...
func (a *UdpAgent) GetEcdhFromConfigFile() (core.Ecdh, error) {
	fileName := filepath.Join(ExeDirPath, "etc", "config.toml")
...
func (a *UdpAgent) UpdateTeeSecret() error {
	fileName := filepath.Join(ExeDirPath, "etc", "dhp.toml")

	ecdh, err := a.GetEcdhFromConfigFile()
	if err != nil {
		return err
	}
...
func (a *UdpAgent) UpdateAgentSecret() error {
	fileName := filepath.Join(ExeDirPath, "etc", "config.toml")
