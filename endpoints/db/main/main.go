		...
		Before: func(c *cli.Context) error {
			if c.String("mode") == "encrypt" {
				if c.String("data-source-type") == "" {
					return fmt.Errorf("--data-source-type is required in encrypt mode")
				} else {
					...
					if c.String("data-source-type") != "stream" {
						if c.String("source") == "" {
							return fmt.Errorf("--source is required when --data-source-type is not stream")
						}
					} else {
