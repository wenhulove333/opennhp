...
type DHPKao struct {
	ConsumerId     string `json:"consumerId"`     // Consumer identifier, it can be email address, mobile number or other unique identifier
	WrappedDataKey string `json:"wrappedDataKey"` // Wrapped data private key
	SpoId          string `json:"spoId"`          // SPO identifier
	KeyWrapper     string `json:"keyWrapper"`     // Key wrapping method: "kas"=KAS public key, "consumer"=data consumer's public key **To Be Removed**
	PolicyBinding  string `json:"policyBinding"`  // Base64-encoded HMAC(HMAC(pao), key) using payload key
	WrappedKey     string `json:"wrappedKey"`     // Base64-encoded payload key encrypted via keyWrapper **To Be Removed**
}
